...
# Common Python layout and per-directory docs; duplicate basenames are expected,
# not hygiene issues.
_DUPLICATE_BASENAME_SKIP = frozenset(
    {"__init__.py", "models.py", "query.py", "types.py", "constants.py", "README.md"}
)
...
def detect_issues(
    ...
    *,
    ...
    for name, paths in by_name.items():
        if name in _DUPLICATE_BASENAME_SKIP:
            continue
        if len(paths) > 1:
            rows.append(
                (
